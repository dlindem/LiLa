...
for lexeme_id in lexemes:
    ...
    for celltype in celltypes_sorted:
        ...
        for cell in lexemes[lexeme_id]['cells']:
            ...
            for row in lexemes[lexeme_id]['cells'][cell]['rows']:
                ...
                if not long_lexeme:
                    ...
                    for claim in claims:
                        claimcreation = lilamorph_api.post('wbcreateclaim', token=lilamorph_api_token, entity=formid, property=claim[0], snaktype="value", bot=1, value=claim[1])
        ...
        if not long_lexeme and rowcount > 560:
            # print("*** Long lexeme. Will skip this lexeme. ***")
            # new_lexeme = None
            # break
            print("*** Long lexeme. Will write 560 forms and then proceed ***")
            lexeme_entity_id = write_lexeme(new_lexeme)
            if lexeme_entity_id:
                print(f"*** Written 560 forms to http://lilamorph.wikibase.cloud/entity/{new_lexeme.id}")
                long_lexeme = lexeme_entity_id
            else:
    ...
    if not long_lexeme:
        lexeme_entity_id = write_lexeme(new_lexeme)
        if lexeme_entity_id:
            print(f"Finished {lexeme_entity_id}")
        else:
            print(f"*** Failed writing.")
        time.sleep(2)
