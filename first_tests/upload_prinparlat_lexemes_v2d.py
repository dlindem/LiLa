...
# for lexeme_id in lexemes:
for lexeme_id in ["a3338","c4706","p0736"]:
    lexeme_count += 1
    ...
    for celltype in celltypes_sorted:
        for cell in lexemes[lexeme_id]['cells']:
            ...
            for row in lexemes[lexeme_id]['cells'][cell]['rows']:
                ...
                # # form creation (version for mediawiki api, used for lexemes bigger than 2MB
                # formdata = {
                #     "representations": {"la": {"language": "la", "value": row['form_normalized']}},
                #     "grammaticalFeatures": cellfeatures[celltype]
                # }
                # formcreation = lilamorph_api.post('wbladdform', token=lilamorph_api_token, format="json",
                #                                   lexemeId=wblexeme_id, bot=1, data=json.dumps(formdata))
                # formid = formcreation['form']['id']
                # print(f'\n{rowcount}: Form creation for ' + wblexeme_id + ': success: ' + formid)
                # time.sleep(0.6)
                # claims = []
                # for flex_id in cell_flex_ids:
                #     claims.append(("P6", f'"{flex_id}"'))
                # claims += [
                #     ("P11", json.dumps({"language": "la", "text": row['orth_form']})),
                #     ("P12", f'"{row["analysed_orth_form"]}"'),
                #     ("P13", f'"{celltype}"')
                # ]
                # for claim in claims:
                #     claimcreation = lilamorph_api.post('wbcreateclaim', token=lilamorph_api_token, entity=formid,
                #                                        property=claim[0], snaktype="value", bot=1, value=claim[1])
                #     if claimcreation['success'] == 1:
                #         print(f'Claim {claim[0]} - {claim[1]}: success.')
                #         time.sleep(.3)
                #     else:
                #         print(f"Failed to write claim {claim[0]} - {claim[1]}")
                #         sys.exit()
    ...
    if len(new_forms) > 0:
        if not lexeme_entity:
            lexeme_entity = lilamorph.lexeme.new(language="Q3", lexical_category="Q5")  # latin verb
            lexeme_entity.lemmas.set(language="la", value=lexemes[lexeme_id]['canonical'])  # wikibase lemma
            lexeme_entity.claims.add(datatypes.Item(prop_nr="P14", value="Q14"))  # collection
            lexeme_entity.claims.add(datatypes.ExternalID(prop_nr="P8", value=lexeme_id))  # prinparlat lexeme id
            lila_lemma_id = lexemes[lexeme_id]['cells'][cell]['rows'][0]['lila_id_lemma']
            lexeme_entity.claims.add(
                datatypes.ExternalID(prop_nr="P1", value=f"lemma/{lila_lemma_id}"))  # lila lemma id


        if len(new_forms) > 500:

            for f in new_forms[:500]:
                lexeme_entity.forms.add(f)
            print(f"Will write {len(new_forms[:500])} forms to lexeme.")
            ...
            if lexeme_entity_id:
                ...
                del new_forms [:500]
                json_lexeme = \
                    requests.get(f"https://lilamorph.wikibase.cloud/wiki/Special:EntityData/{lexeme_entity_id}.json").json()[
                        'entities'][
                        wblexeme_id]
                json_lexeme['forms'] = []
...
# nice code:
            ...
            #     lilamorph_api_token = csrfquery['query']['tokens']['csrftoken']
            #     print("Got fresh CSRF token for lilamorph.wikibase.cloud.")
            #     json_lexeme = new_lexeme.get_json()
            #     rowcount = 0
            #     for form in json_lexeme['forms']:
            #         rowcount += 1
            #         formdata = {
            ...
            #                                       lexemeId=wblexeme_id, bot=1, data=json.dumps(formdata))
            #         formid = formcreation['form']['id']
            #         print(f'[{rowcount}/{len(json_lexeme['forms'])}]: Form creation for ' + wblexeme_id + ': success: ' + formid)
            #         time.sleep(0.41)
            #         for prop in form['claims']:
            #             for claim in form['claims'][prop]:
            #                 if claim['mainsnak']['datatype'] == "wikibase-item":
            #                     target_nid = claim['mainsnak']['datavalue']['value']['numeric-id']
            #                     claimvalue = json.dumps({"entity-type": "item", "numeric-id": target_nid})
            #                 elif claim['mainsnak']['datatype'] == "monolingualtext":
            #                     claimvalue = json.dumps(claim['mainsnak']['datavalue']['value'])
            #                 elif claim['mainsnak']['datatype'] == "string":
            #                     claimvalue = f'"{claim['mainsnak']['datavalue']['value']}"'
            #                 else:
            #                     print(f"datatype {claim['mainsnak']['datatype']} not implemented...")
            #                     sys.exit()
            #                 claimcreation = lilamorph_api.post('wbcreateclaim', token=lilamorph_api_token, entity=formid,
            #                                                property=prop, snaktype="value", bot=1, value=claimvalue)
            #                 if claimcreation['success'] == 1:
            #                     print(f'Claim {prop} - {claimvalue}: success.')
            #                     time.sleep(.3)
            #                 else:
            #                     print(f"Failed to write claim {prop} - {claimvalue}")
            #                     sys.exit()
            #     return lexeme_entity_id
            #
            # print(errortext)
            # print("Failed to write data to Wikibase.")
            # sys.exit()