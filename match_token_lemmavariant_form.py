...
with open('mappings/lilalemmas_lemmavariants.csv') as file:
    lemma_variants = {}
    rows = csv.DictReader(file, delimiter="\t")
    for row in rows:
        lemma_variants[row['lemma']] = row['variant']
...
with open("data/ITTB_verb_tokendata_for_upload.csv") as file:
    ...
    for row in tokenrows:
        tokenrowcount += 1
        print(f"\n[{tokenrowcount}] Now processing token: https://lilamorph.wikibase.cloud/entity/{token_items[row['token']]} - {row['token']}")
        if token_items[row['token']] in done_links: # do not process tokens that already are linked to a form using the standard methods (not through lemma variants)
            print("Skip: Has a form link already.")
            continue
        ...
        print(f"[{tokenrowcount}] Now processing lemma IDs: {lila_lemmas}...")
        for lila_lemma in lila_lemmas:

            if lila_lemma not in lemma_variants:
                continue
            variant = lemma_variants[lila_lemma]

            if variant not in lemmadict:
                print(f"Retrieving form data for LiLa Lemma variant '{variant}' from Wikibase...")
                lemmadict[variant] = get_wikibase_forms(variant)

            if len(lemmadict[variant]) == 0:
                if variant not in not_found:
                    not_found.append(variant)
                    print("Did not find this lemma on Wikibase.")
                    with open('data/lemma_not_found_lexemedict_variants_v3.csv', 'a') as outfile:
                        outfile.write(f"{variant}\n")
                continue

            # matching algorithm
            for formdata in lemmadict[variant]:
                ...
                if formdata['form_rep']['value'] == row['tokenLabel'].replace("v", "u"):
                    ...
                    if match:
                        ...
                        with open('data/matching_tokens_lexemedict_variants_v3.csv', 'a') as outfile:
                            outfile.write(
                                f"{row['token']}\t{row['tokenLabel']}\t{token_items[row['token']]}\t{formdata['form']['value']}\t{variant}\t{'|'.join(token_features)}\n")

        if not match_for_token:
            with open('data/not_matching_tokens_lexemedict_variants_v3.csv', 'a') as outfile:
                outfile.write(
                    f"{row['token']}\t{row['tokenLabel']}\tNO MATCH\t{variant}\t{'|'.join(token_features)}\n")
